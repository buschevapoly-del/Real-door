...
FIELD_PURPOSES = {
    "application_summary": {
        ...
        "household_size": (
            ...
            "calculation is the value you enter separately above, not this extracted field."
        ),
...
FLASH_MESSAGES = {
    ...
    "doc_saved": "Document changes saved.",
...
def set_size(household_id: str, household_size: int = Form(...)):
    ...
    return RedirectResponse(url=f"/household/{household_id}?flash=size_saved", status_code=303)
...
async def upload_document(
    ...
    household_id: str,
    file: UploadFile,
    document_type_hint: str = Form(""),
    consent: str = Form(""),
):
    household = storage.get_household(household_id)
    if not household["consent_given"] and consent != "1":
        return _render_household(
            request,
            household_id,
            consent_error=(
                "Please check the consent box before uploading a document -- "
                "see \"How this works\" for what gets extracted and why."
            ),
        )
    if consent == "1":
...
async def save_document(request: Request, household_id: str, document_id: str):
    ...
    return RedirectResponse(url=f"/household/{household_id}?flash=doc_saved", status_code=303)
...
def delete_document(household_id: str, document_id: str):
    ...
    return RedirectResponse(url=f"/household/{household_id}?flash=doc_deleted", status_code=303)
